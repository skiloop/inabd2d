...
void SaveCapField(const int timestep)
{
    ...
    if ( scnt % skipstep == 0 /*&& cnt <=SAVE_LEAP*/) {
        ...
        if(IsTEx){
			...
			file[0] = 'h';	file[1] = 'z';	CaptDataNoPML(cnt%10,file,Hz_s,tpis,tpie,tpjs,tpje);
			saveEfieldCenterTMz(timestep);
		}
		file[0] = 'n';file[1] = 'e';CaptDataMNoPML(cnt%10,file,ne,4,tpis*m,tpie*m,tpjs*m,tpje*m);
		file[0] = 'n';file[1]='c';CaptDataCenter(cnt%10,file,ne,ne.ny/2,tpis*m,tpie*m);	
		file[0] = 'e';file[1] = 'm';CaptDataMNoPML(cnt%10,file,Erms,4,tpis*m,tpie*m,tpjs*m,tpje*m);
		file[0] = 'm';file[1]='c';CaptDataCenter(cnt%10,file,Erms,Erms.ny/2,tpis*m,tpie*m);
